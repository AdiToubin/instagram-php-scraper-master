...
flowchart TB
    ...
    subgraph Database["💾 Supabase Database"]
        raw_story[("story_raw<br/>סטוריז גולמיים")]
...
```powershell
# התקנת uv (פעם אחת — מנהל חבילות Python)
pip install uv

# התקנת תלויות PHP (מתוך תיקיית scraper/)
cd scraper
composer install
cd ..

# התקנת תלויות Python (מתוך תיקיית analyzer/)
cd analyzer
uv sync
cd ..
```
...
```powershell
php scraper/get_user_ids.php --file influencers.txt
```
...
php scraper/stories_with_stickers.php USER_ID

# מספר משפיעניות
php scraper/batch_stories.php --file user_ids.txt
```
...
python analyzer/run_daily_stories.py

# עם פרמטרים מותאמים
python analyzer/run_daily_stories.py --min-delay 30 --max-delay 120 --dry-run
```

### 5️⃣ עיבוד וסיווג עם AI

```powershell
python analyzer/run_daily.py
```

**מה הסקריפט עושה:**
1. ✅ טוען מיפוי שמות משתמש מ-JSON
2. 📥 שולף סטוריז מ-`story_raw`
...
│
├── scraper/                          # 📄 PHP — שליפת סטוריז
│   ├── src/InstagramScraper/         # ספריית הליבה
│   ├── examples/                     # דוגמאות API
│   ├── cookies/                      # סשן אינסטגרם
...
│
├── analyzer/                         # 🐍 Python — ניתוח AI
│   ├── ai_sidecar/                   # מודול vector + memory
│   ├── run_daily.py                  # עיבוד AI וסיווג ← ראשי
│   ├── run_daily_stories.py          # אוטומציה יומית
│   ├── run_daily_paddle.py           # גרסת Paddle
│   ├── generate_influencer_report.py
│   ├── check_sidecar_status.py
│   ├── pyproject.toml                # תלויות Python
│   └── uv.lock
│
├── archive/                          # ניסיונות — לא בשימוש פעיל
├── db/                               # migrations.sql
├── docs/                             # תיעוד נוסף
├── logs/                             # קבצי לוג
│
├── influencers.txt                   # 📊 רשימת משפיעניות
├── user_ids_*.json                   # מיפוי username ↔ user_id
├── .env                              # ⚙️ משתני סביבה
└── README.md                         # המסמך הזה
```
...
```mermaid
erDiagram
    story_raw ||--o{ relevant_story : "מסווג ל"
    story_raw ||--o{ story_recommendations : "מסווג ל"
