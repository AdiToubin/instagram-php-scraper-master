...
def _safe_name(row: Dict[str, Any], result: Dict[str, Any]) -> str:
    ...
    3) fallback ל-user_id (עדיף מ-"unknown")
    """
    ...
    # אם לא מצאנו username, השתמש ב-user_id
...
def already_in_relevant(media_id: str) -> bool:
    ...
    # Check story_recommendations table
    url = f"{SUPABASE_REST}/{REC_TABLE}"
    params = {"select": "media_id", "media_id": f"eq.{media_id}", "limit": "1"}
    r = sb_get(url, params)
    if r.status_code >= 400:
        print("⚠️ recommendations lookup error:", r.status_code, r.text)
    else:
        if r.json():
            return True
...
def generate_performance_report():
    ...
    try:
        ...
        # קופונים ולינקים
        url = f"{SUPABASE_REST}/{REL_TABLE}"
        r = sb_get(url, {"select": "user_id,media_id,url,coupon", "limit": "10000"})
        relevant = r.json() if r.status_code == 200 else []
        
        # המלצות
        url = f"{SUPABASE_REST}/{REC_TABLE}"
        r = sb_get(url, {"select": "user_id,media_id", "limit": "10000"})
        recommendations = r.json() if r.status_code == 200 else []
